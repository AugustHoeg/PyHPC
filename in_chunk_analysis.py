...
if __name__ == "__main__":
    ...
    for patch_size in [16, 32, 64, 128, 256]:
        ...
        for chunk_size in [64, 128, 256]:
            ...
            if patch_size >= chunk_size:
                print(f"Skipping chunk size {chunk_size} for patch size {patch_size} as it is not larger")
                continue
            ...
            out_file = f"results/in_chunk_analysis/in_chunk_analysis_patch_size_{patch_size}_chunk_size_{chunk_size}_{current_time}.txt"
            save_results(result_dict, output_file=out_file)

            print(f"Results saved to {out_file}")

        plot_time_plots_multi(result_dict_list, save_path="figures", filename_prefix=f"in_chunk_analysis_patch_size_{patch_size}")