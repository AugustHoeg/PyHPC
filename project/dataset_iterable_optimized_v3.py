...
def extract_patch_levels(data, group_pair, patch_size=(32, 32, 32), up_factor=4):
    volume_L = data[group_pair['L']]
    volume_H = data[group_pair['H']]

    # TODO: make it so a patch of size "patch_size" and "patch_size"*"up_factor" is always returned
    valid_shape = np.maximum((np.array(volume_L.shape) - patch_size), (1, 1, 1))
    start = np.random.randint(0, valid_shape)
    end = start + patch_size
    out_dict = {'L': volume_L[start[0]:end[0], start[1]:end[1], start[2]:end[2]],
                'H': volume_H[start[0]*up_factor:end[0]*up_factor,
                              start[1]*up_factor:end[1]*up_factor,
                              start[2]*up_factor:end[2]*up_factor]}
...
class ZarrProducer():
    ...
    def _worker_process(self, id):
        ...
        while not self.stop_event.is_set():
            ...
            patch = self._sample_data(z, group_pair, self.patch_shape)
...
class ZarrIterableDataset(IterableDataset):
    ...
    def _generate_patch(self, worker_data):
        ...
        name = random.choices(list(worker_data.keys()), weights=w)[0]  # random name
